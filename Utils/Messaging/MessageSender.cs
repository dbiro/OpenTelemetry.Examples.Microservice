        ...
        public string SendMessage(string user)
        {
            ...
            //using (var activity = Activity.Current)
            {
                ...
                try
                {
                    ...
                    logger.LogInformation($"Activity.IsAllDataRequested={activity?.IsAllDataRequested}");
                    activity?.SetTag("nexogen.user", user);
                    ...
                    var body = $"Published message: DateTime.Now = {DateTime.Now}, user: {user ?? "-"}.";
                    ...
                    activity?.AddEvent(new ActivityEvent("Message has been sent.", DateTimeOffset.UtcNow));
                ...
                catch (Exception ex)
                {
                    ...
                    Activity.Current?.AddEvent(new ActivityEvent("Message published failed", DateTimeOffset.UtcNow));
                    throw;
                }
            }
